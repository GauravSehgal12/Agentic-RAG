...
class ChatIngestor:
    def __init__( self,
        ...
        try:
            ...
            log.info(event="ChatIngestor initialized",
                     session_id=self.session_id,
                     temp_dir=str(self.temp_dir),
                     faiss_dir=str(self.faiss_dir),
                     sessionized=self.use_session)
        except Exception as e:
            log.error(event="Failed to initialize ChatIngestor", error=str(e))
    ...
    def _split(self, docs: List[Document], chunk_size=1000, chunk_overlap=200) -> List[Document]:
        ...
        log.info(event="Documents split", chunks=len(chunks), chunk_size=chunk_size, overlap=chunk_overlap)
        return chunks
    ...
    def built_retriver( self,
        ...
        *,
        ...
        try:
            ...
            log.info(event="FAISS index updated", added=added, index=str(self.faiss_dir))

            # Configure search parameters based on search type
            search_kwargs: Dict[str, Any] = {"k": int(k)}
            ...
            if search_type == "mmr":
                ...
                log.info(event="Using MMR search", k=k, fetch_k=fetch_k, lambda_mult=lambda_mult)

            return vs.as_retriever(search_type=search_type, search_kwargs=search_kwargs)

        except Exception as e:
            log.error(event="Failed to build retriever", error=str(e))
