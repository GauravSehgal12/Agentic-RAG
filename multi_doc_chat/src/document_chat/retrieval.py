...
from multi_doc_chat.prompt.prompt_library import PROMPT_REGISTRY
...
class ConversationalRAG:
    ...
    def __init__(self, session_id: Optional[str], retriever=None):
        try:
            ...
            log.info(event="ConversationalRAG initialized", session_id=self.session_id)
        except Exception as e:
            log.error(event="Failed to initialize ConversationalRAG", error=str(e))
    ...
    def invoke(self, user_input: str, chat_history: Optional[List[BaseMessage]] = None) -> str:
        ...
        try:
            ...
            try:
                ...
                log.error(event="Invalid chat answer", error=str(ve))
                raise DocumentPortalException("Invalid chat answer", sys)
            log.info(
                event="Chain invoked successfully",
                session_id=self.session_id,
                user_input=user_input,
            ...
            log.error(event="Failed to invoke ConversationalRAG", error=str(e))
    ...
    def _load_llm(self):
        try:
            ...
            log.info(event="LLM loaded successfully", session_id=self.session_id)
            return llm
        except Exception as e:
            log.error(event="Failed to load LLM", error=str(e))
    ...
    def _build_lcel_chain(self):
        try:
            ...
            log.info(event="LCEL graph built successfully", session_id=self.session_id)
        except Exception as e:
            log.error(event="Failed to build LCEL chain", error=str(e), session_id=self.session_id)
