    ...
    try:
        for p in paths:
            ...
            if ext == ".pdf":
                ...
                log.warning(event="unsupported_extension_skipped", message="Unsupported extension skipped", path=str(p))
                continue
            docs.extend(loader.load())
        log.info(event="documents_loaded", message="Documents loaded", count=len(docs))
        return docs
    except Exception as e:
        log.error(event="document_loading_failed", message="Failed loading documents", error=str(e))
